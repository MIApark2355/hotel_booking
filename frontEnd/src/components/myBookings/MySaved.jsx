...
import "./mySaved.css"
   ...
   export default function SavedList(username) {
    ...
    useEffect(() => {
      async function getRecords() {
        ...
        console.log("from saved list", records);
      };
    ...
    async function deleteRecord(id) {
      console.log(id);
      await fetch(`http://localhost:5000/savedlist/${id}`, {
        method: "DELETE"
      });
    ...
    return (
      ...
      <div className="all">
        ...
        <h1 className="listHeading">Your Hotel Saved List</h1>
        </div>
        <table className="table table-striped" style={{ marginTop: 20 }}>
        <tbody>{SavedList()}</tbody>
        </table>
      </div>
      </div>
