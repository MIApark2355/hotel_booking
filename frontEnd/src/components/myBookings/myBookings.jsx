...
import "./myBookings.css"
   ...
   const Record = (props) => (
    ...
    <div className="hotelTxt1">
        <h1 className="hotelName">{props.record.hotel}</h1>
        <div className="hotelTxt2">
            {`Your move in date is on ${props.record.movein}`}
        </div>
        <div className="hotelTxt2">
            {`Your move out date is on ${props.record.moveout}`}
    </div>
    <div className="hotelTxt2">
            {`${props.record.people} people are staying`}
    </div>
  
    <div className="hotelTxt2">
    <div className="hotelRating">
      {props.record.ratings === null ? <span>You need to rate</span> : <span>Your Rating On this hotel</span>
      }
      <button>{props.record.ratings}</button>
    </div>
    <div className="hotelDetail">
      <div className="btns">
      <Link className="editBtn btn-link" to={`/reservations/edit/${props.record._id}`}>Edit</Link>
        <button className="deleteBtn btn-link"
          onClick={() => {
   ...
   export default function BookingList(username) {
    const [records, setRecords] = useState([]);
    const user = username;
    const newPerson = {name: username};
    ...
    return (
      ...
      <div className="all">
        ...
        <h1 className="listHeading">Your Hotel Booking List</h1>
        </div>
